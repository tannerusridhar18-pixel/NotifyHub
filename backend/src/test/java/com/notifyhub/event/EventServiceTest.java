...
@ExtendWith(MockitoExtension.class) class EventServiceTest {
 ...
 @Test void rejectsEndBeforeStart(){Instant now=Instant.now();assertThatThrownBy(()->service.create(new EventService.Request("T","D","L",now,now,TargetType.GLOBAL,null,null,null,null),"a")).isInstanceOf(ResponseStatusException.class).hasMessageContaining("after start");}
