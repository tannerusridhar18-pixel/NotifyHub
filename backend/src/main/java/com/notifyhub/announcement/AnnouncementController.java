...
public class AnnouncementController {
    ...
    @GetMapping("/urgent") public ResponseEntity<ApiResponse<PageResponse<AnnouncementService.AnnouncementDto>>> urgent(Authentication a, @RequestParam(defaultValue="0") @Min(0) int page, @RequestParam(defaultValue="20") @Min(1) @Max(100) int size) { return ResponseEntity.ok(ApiResponse.ok(!authenticated(a) ? service.urgent(page, size) : service.urgent(a.getName(), page, size))); }
    ...
    @DeleteMapping("/{id}") public ResponseEntity<ApiResponse<Void>> delete(@PathVariable Long id, Authentication a) { service.delete(id, a.getName()); return ResponseEntity.ok(ApiResponse.message("Announcement deleted.")); }
}
