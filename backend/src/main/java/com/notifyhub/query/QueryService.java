...
public class QueryService {
    ...
    public QueryService(QueryRepository r, UserRepository u) {
        repo = r;
        users = u;
    }
    ...
    public PageResponse<QueryDto> listMyQueries(String username, int page, int size) {
        ...
        Pageable p = PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "createdAt"));
        Page<CampusQuery> x = repo.findByEmailIgnoreCase(user.getEmail(), p);
        return PageResponse.from(x.map(QueryDto::from));
    }
    ...
    public record QueryDto(Long id, String name, String email, String department, String subject, String message, QueryStatus status, String adminResponse, Instant createdAt, Instant answeredAt) {
        static QueryDto from(CampusQuery q) {
            return new QueryDto(q.getId(), q.getName(), q.getEmail(), q.getDepartment(), q.getSubject(), q.getMessage(), q.getStatus(), q.getAdminResponse(), q.getCreatedAt(), q.getAnsweredAt());
        }
    }
}
