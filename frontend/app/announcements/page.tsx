...
export default function AnnouncementsPage(){
 ...
 const load=(next?: {q:string;category:string;department:string}|MouseEvent<HTMLButtonElement>)=>{const filters=next&&"q" in next?next:{q,category,department};setLoading(true);setError("");announcements(filters).then(x=>setItems(x.content)).catch(e=>setError(e.message)).finally(()=>setLoading(false));};
 useEffect(()=>{void announcements({}).then(x=>setItems(x.content)).catch(e=>setError(e.message)).finally(()=>setLoading(false));},[]);
