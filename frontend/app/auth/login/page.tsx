...
export default function UserLogin(){
 ...
 async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");try{const x=await login(email,password);router.push(x.role==="ADMIN"?"/admin/dashboard":"/")}catch(e){setError(e instanceof Error?e.message:"Login failed.")}finally{setBusy(false)}}
 ...
 <p className="muted auth-switch">Have an invitation? <Link href="/auth/register">Complete registration</Link></p><p className="muted auth-switch"><Link href="/admin">Admin portal</Link></p>
 </form></div>
}
