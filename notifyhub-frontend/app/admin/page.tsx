...
export default function AdminLogin(){
 ...
 async function submit(e:React.FormEvent){e.preventDefault();setBusy(true);setError("");try{const x=await login(email,password);localStorage.setItem("notifyhub_access_token",x.accessToken);localStorage.setItem("notifyhub_refresh_token",x.refreshToken);localStorage.setItem("notifyhub_username",x.username);router.push("/admin/dashboard")}catch(e){setError(e instanceof Error?e.message:"Login failed.")}finally{setBusy(false)}}
